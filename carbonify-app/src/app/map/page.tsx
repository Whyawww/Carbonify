...
export default function MapPage() {
  ...
  const fetchEcoPoints = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/ecopoints/?${params.toString()}`,
      );
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        console.error(
          'Gagal mendapatkan lokasi. Pencarian berbasis lokasi dinonaktifkan.',
        );
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="p-[1px] bg-gradient-to-r from-green-400/30 to-cyan-400/30 rounded-2xl mb-8">
          <form
            ...
            className="p-4 bg-gray-900/80 backdrop-blur-lg rounded-2xl flex flex-col md:flex-row gap-4 items-center"
          >
            <input
              type="text"
              name="search"
            ...
            >
              ...
              <option value="Thrift Store / Pakaian Bekas">
                Thrift Store / Pakaian Bekas
              </option>
              <option value="Bulk Store / Zero Waste">
                Bulk Store / Zero Waste
              </option>
            ...
            <button
              type="submit"
              className="w-full md:w-auto font-bold py-2 px-4 rounded-md transition-all duration-300 text-white bg-gradient-to-r from-green-500 to-cyan-500 hover:from-green-600 hover:to-cyan-600"
            >
              Cari
            </button>
          </form>
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  {ecoPoints.length > 0 ? (
                      ...
                      >
                        ...
                        {point.distance !== undefined && (
                          <p className="text-xs text-cyan-400">
                            ~{point.distance.toFixed(2)} km dari Anda
                          </p>
                        )}
                    ...
                    <p>
                      Tidak ada lokasi yang ditemukan. Coba perbesar radius
                      pencarian.
                    </p>
                  )}
                </div>
          ...
          <div className="lg:col-span-2 p-[1px] bg-gradient-to-r from-green-400/30 to-cyan-400/30 rounded-2xl h-[600px]">
            <div className="bg-gray-900/80 backdrop-blur-lg rounded-2xl h-full overflow-hidden">
              <Map
                ...
                radiusInKm={parseInt(filters.radius)}
              />
            </div>
          </div>
        </div>
