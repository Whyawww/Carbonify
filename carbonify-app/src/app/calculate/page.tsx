  ...
  useEffect(() => {
    const fetchChoices = async () => {
      try {
        ...
        const [listrikRes, transRes, makananRes, bahanbakarRes] = await Promise.all([
        ...
        if (!listrikRes.ok || !transRes.ok || !makananRes.ok || !bahanbakarRes.ok) { // 
          ...
          <div className="text-center md:text-left flex flex-col justify-between">
            <div>
              ...
              <p className="text-5xl lg:text-6xl font-bold text-white mb-4">
                ...
                <span className="text-xl lg:text-2xl align-baseline ml-2">
                  kg CO₂e / bulan
                </span>
              </p>
            ...
            <div className="mt-8 flex flex-col sm:flex-row md:flex-col lg:flex-row gap-4">
              <Link
                href={actionLink}
                className={`w-full lg:w-auto font-bold py-3 px-6 rounded-md transition-colors ${actionButtonClass}`}
              >
                {actionText}
              </Link>
              <button
                onClick={resetCalculator}
                className="w-full lg:w-auto bg-gray-600 hover:bg-gray-700 text-white font-bold py-3 px-4 rounded-md"
              >
                Hitung Ulang
              </button>
            </div>
          ...
          <div className="mt-8 md:mt-0 md:border-l md:border-gray-700 md:pl-8">
            <div className="border-b border-gray-700 pb-4">
              ...
              <div className="text-lg text-gray-300 w-full space-y-3">
                <p
                  className={`flex justify-between items-center ${categoriesOverLimit.includes('Listrik') ? 'text-yellow-400 font-semibold' : ''}`}
                >
                  <span>⚡️ Listrik</span>
                  <strong>{breakdown.listrik.toFixed(2)} kg CO₂e</strong>
                </p>
                <p
                  className={`flex justify-between items-center ${categoriesOverLimit.includes('Transportasi') ? 'text-yellow-400 font-semibold' : ''}`}
                >
                  <span>🚗 Transportasi</span>
                  <strong>{breakdown.transportasi.toFixed(2)} kg CO₂e</strong>
                </p>
                <p
                  className={`flex justify-between items-center ${categoriesOverLimit.includes('Konsumsi') ? 'text-yellow-400 font-semibold' : ''}`}
                >
                  <span>🍔 Konsumsi</span>
                ...
                {breakdown.bahan_bakar > 0 && (
                  <p className={`flex justify-between items-center ${categoriesOverLimit.includes('Bahan Bakar') ? 'text-yellow-400 font-semibold' : ''}`}>
          ...
          <div className="p-[1px] bg-gradient-to-r from-green-400/30 to-cyan-400/30 rounded-2xl max-w-3xl mx-auto">
            ...
            >
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <select name="bahan_bakar_id" value={values.bahan_bakar_id} onChange={handleChange} required className="w-full bg-gray-800/50 border border-gray-700 rounded-md p-3 focus:ring-green-500 focus:border-green-500">
                    <option value="">Pilih Jenis Bahan Bakar</option>
                    {choices.bahan_bakar.map((c) => (<option key={c.id} value={c.id}>{c.jenis_bahan_bakar}</option>))}
                  </select>
                  <input type="number" name="bahan_bakar_liter" value={values.bahan_bakar_liter} placeholder="Jumlah (Liter atau Kg / bulan)" onChange={handleChange} required className="w-full bg-gray-800/50 border border-gray-700 rounded-md p-3 focus:ring-green-500 focus:border-green-500"/>
                </div>
              </div>
